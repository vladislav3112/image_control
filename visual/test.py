...
def rearrangeCorners(corners):
    ...
    for i in range(4):
        ...
        if p[0] <= center[0] and p[1] >= center[1]:
            ...
            break
    rearranged = np.array(
        [corners[idx], corners[idx - 1], corners[idx - 2], corners[idx - 3]]
    )
    return rearranged
...
c = p.loadURDF("combined/aruco.urdf", (0.0, 0.0, 0.0), useFixedBase=True)
...
distCoeffs = np.array([])
cameraMatrix = np.array([[f, 0, IMG_HALF], [0, f, IMG_HALF], [0, 0, 1]])
objPoints = np.array(
    [
